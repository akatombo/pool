require('chai').should();
var Pool = require('../build/build');

describe('new Pool(provider, max)', function () {
	var pool;
	...
	beforeEach(function () {
		pool = new Pool(customProvider, 5);
	});

	it('', function () {});


	describe('#acquire([argument]*)', function () {
		it('', function () {});
	});

	describe('#release(instance, [argument]*)', function () {
		it('', function () {});
	});
	...
	describe('#max', function () {
		it('default value should be equal max javascript number', function () {
			var defaultPool = new Pool();
			defaultPool.max.should.be.equal(Number.MAX_VALUE);
		});
		it('should be equal max instance number', function () {
			var pool = new Pool(null, 20);
			pool.max.should.be.equal(20);
		});
	});
});
